...
public class PlayerManager {
    ...
    public String displayAll() {
        if(playerList.isEmpty()) {
            return "There is no record of players available.";
        }
        StringBuilder playerbuilder = new StringBuilder();
        for(SoccerPlayer player : playerList) {
            playerbuilder.append(player.displayPlayerRecord());
        }
     ...
     * return: String (Updated from void)
     * purpose: loads players from a .txt file into the list and returns a status message
     */
    public String fileUpload(String soccerDataFile) {
        if (!soccerDataFile.endsWith(".txt")) {
            return "Your file is not a .txt file. Please upload only .txt files.";
        }

        File newFile = new File(soccerDataFile);

        if (!newFile.exists()) {
            return "The file '" + soccerDataFile + "' was not found. Please try again";
        }
        ...
        try {
            ...
            while (scan.hasNextLine()) {
                ...
                String team = data[3].trim();
                int matches = Integer.parseInt(data[4].trim());
                int goals = Integer.parseInt(data[5].trim());
                int assists = Integer.parseInt(data[6].trim());
                int minutes = Integer.parseInt(data[7].trim());
            ...
            scan.close();
            return "Your list of " + count + " players have been loaded successfully.";

        } catch (Exception e) {
            return "There was an error reading the file. Verify if it is formatted correctly.";
        }
    }
